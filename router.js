...
var Adminuser = require('./models/user_admin')
...
router.post('/register',function(req,res){
    ...
    //             return res.send(`用户名或者密码已存在，请重试`)
    //         }
    //         new Adminuser(body).save(function(err,student){
    //             if(err){
    //                 console.log(err)
    //                 return res.status(500).json({
    //                     err_code:500,
...
router.get('/manager/student/edit', function (req, res) {
    Grade.findById(req.query.id), function (err, grade) {
      if (err) {
        return res.status(500).send('Server error.')
      }
...
// 删除教师数据(/manager/teacher/delete) 已完成
